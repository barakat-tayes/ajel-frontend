...
export default function AdminDashboard() {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await api.get(
          `/admin/lookup-by-phone?userType=${encodeURIComponent(
            tempReset.userType,
  ...
  const restaurantProvinces = useMemo(
    () =>
      Array.from(
        new Set((ledger || []).map((r) => r.province).filter(Boolean)),
  ...
  const pendingRequests = useMemo(
    () => (pending.restaurants?.length || 0) + (pending.drivers?.length || 0),
    [pending],
  );
  const restaurantsDue = useMemo(
    () =>
      filteredLedger.reduce((acc, r) => acc + Number(r.current_due || 0), 0),
    [filteredLedger],
  );
  ...
  const doAction = async (id, action) => {
    ...
    if (action === "pay") {
      ...
      if (value <= 0) {
        await Swal.fire({
          icon: "warning",
          title: "أدخل مبلغ تسديد صحيح",
          confirmButtonColor: "#e31e24",
        });
        return;
      }
      await api.put(`/admin/restaurants-ledger/${id}/pay`, {
        paid_amount: value,
      });
      setPayValues((prev) => ({ ...prev, [id]: "" }));
    }
    if (action === "remind")
      await api.put(`/admin/restaurants-ledger/${id}/remind`);
    if (action === "warn")
      await api.put(`/admin/restaurants-ledger/${id}/toggle-warning`);
    if (action === "suspend")
      await api.put(`/admin/restaurants-ledger/${id}/toggle-suspend`);
    if (action === "deleteRestaurant")
      await api.delete(`/admin/restaurants/${id}`);
    if (action === "suspend24h")
      await api.put(`/admin/drivers/${id}/suspend-24h`);
    if (action === "suspend72h")
      await api.put(`/admin/drivers/${id}/suspend-72h`);
    if (action === "toggleDriverSuspend")
      await api.put(`/admin/drivers/${id}/toggle-suspend`);
    if (action === "clearDriverSuspend")
      await api.put(`/admin/drivers/${id}/clear-suspend`);
  ...
  const driverActionLabel = (driver, mode) => {
    ...
    if (mode === "24h")
      return suspended && reason === "24h" ? "إلغاء إيقاف يوم" : "إيقاف يوم";
    if (mode === "72h")
      return suspended && reason === "72h"
        ? "إلغاء إيقاف 3 أيام"
        : "إيقاف 3 أيام";
    return suspended && (!driver?.suspended_until || reason === "manual")
      ? "تفعيل"
      : "إيقاف";
  };
  ...
  const updatePerOrderFee = async () => {
    ...
    await api.put("/admin/settings", {
      per_order_fee: Number(result.value || 0),
    });
    await Swal.fire({
      icon: "success",
      title: "تم حفظ التسعيرة",
      confirmButtonColor: "#e31e24",
    });
    await load();
  };
  ...
  const resetTempPassword = async () => {
    if (!tempReset.phone || !tempReset.tempPassword) {
      await Swal.fire({
        icon: "warning",
        title: "أدخل رقم الهاتف وكلمة المرور المؤقتة",
        confirmButtonColor: "#e31e24",
      });
      return;
    }
  ...
  const requestsRows = [
    ...
    (x) =>
      selectedProvince === ALL_PROVINCES || x.province === selectedProvince,
  );

  const openMap = (x) => {
    const link =
      x.location_link ||
      (x.location_lat && x.location_lng
        ? `https://www.google.com/maps?q=${x.location_lat},${x.location_lng}`
        : "");
      ...
      >
        ...
        <button
          ...
          className={`${styles.sidebarTab} ${
            view === "restaurants" ? styles.sidebarTabActive : ""
          }`}
        >
          التجار
        </button>
        <button
          ...
          className={`${styles.sidebarTab} ${
            view === "drivers" ? styles.sidebarTabActive : ""
          }`}
        >
          السائقون
        </button>
        <button
          ...
          className={`${styles.sidebarTab} ${
            view === "requests" ? styles.sidebarTabActive : ""
          }`}
        >
          طلبات الانضمام
          {pendingRequests > 0 ? (
            <span style={badgeStyle}>{pendingRequests}</span>
          ) : null}
        </button>
        <button
          onClick={() => switchView("settings")}
          className={`${styles.sidebarTab} ${
            view === "settings" ? styles.sidebarTabActive : ""
          }`}
        >
          إعدادات الأدمن
        </button>
              ...
              {filteredLedger.map((r) => (
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ fontWeight: 900, color: "#e31e24" }}>
                      {asIqd(r.current_due)}
                    </div>
                  </div>
                  <div
                  ...
                  >
                    ...
                    />
                    <button
                      onClick={() => doAction(r.id, "pay")}
                      style={miniBtn("#059669", "#fff")}
                    >
                      تسديد
                    </button>
                    <button
                      onClick={() => doAction(r.id, "remind")}
                      style={miniBtn("#f59e0b", "#111")}
                    >
                      تنبيه
                    </button>
                    <button
                      onClick={() => doAction(r.id, "warn")}
                      style={miniBtn(
                        r.suspension_warning_at ? "#dc2626" : "#fb7185",
                        "#fff",
                      )}
                    >
                    ...
                    <button
                      ...
                      style={miniBtn(
                        r.status === "suspended" ? "#2563eb" : "#dc2626",
                        "#fff",
                      )}
                    >
                    ...
                    <button
                      onClick={() => doAction(r.id, "deleteRestaurant")}
                      style={miniBtn("#7f1d1d", "#fff")}
                    >
                      حذف التاجر
                    </button>
                  </div>
              ...
              {driversFiltered.map((d) => (
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        doAction(d.id, driverActionKey(d, "manual"))
                      }
                      style={miniBtn(
                        d.account_status === "suspended" &&
                          (!d.suspended_until ||
                            d.suspension_reason === "manual")
                          ? "#2563eb"
                          : "#dc2626",
                        "#fff",
                      )}
                    ...
                    <button
                      onClick={() => doAction(d.id, "deleteDriver")}
                      style={miniBtn("#7f1d1d", "#fff")}
                    >
                      حذف
                    </button>
          ...
          {view === "requests" && (
            <div style={{ display: "grid", gap: 8 }}>
              ...
              >
                ...
                >
                  <select
                    value={tempReset.userType}
                    onChange={(e) =>
                      setTempReset((p) => ({ ...p, userType: e.target.value }))
                    }
                    style={{
                      minWidth: 130,
                      borderRadius: 8,
                  ...
                  <input
                    value={tempReset.phone}
                    onChange={(e) =>
                      setTempReset((p) => ({ ...p, phone: e.target.value }))
                    }
                ...
                {lookupUser ? (
                  ...
                  >
                    <div>
                      <b>الاسم:</b> {lookupUser.name || "-"}
                    </div>
                    {tempReset.userType === "restaurant" ? (
                      <div>
                        <b>المالك:</b> {lookupUser.owner_name || "-"}
                      </div>
                    ) : (
                      <div>
                        <b>المركبة:</b> {lookupUser.vehicle_type || "-"}
                      </div>
                    )}
                    <div>
                      <b>الهاتف:</b> {lookupUser.phone || "-"}
                    </div>
                    <div>
                      <b>المحافظة:</b> {lookupUser.province || "-"}
                    </div>
                    {tempReset.userType === "restaurant" ? (
                      <div>
                        <b>العنوان:</b> {lookupUser.address || "-"}
                      </div>
                    ) : (
                      <div>
                        <b>اللوحة:</b> {lookupUser.vehicle_plate || "-"}
                      </div>
                    )}
                  </div>
                ) : null}
                ...
                >
                  <div
                    style={{
                      ...
                      background: x.t === "restaurant" ? "#dcfce7" : "#dbeafe",
                      color: x.t === "restaurant" ? "#166534" : "#1e40af",
                      border: `1px solid ${
                        x.t === "restaurant" ? "#86efac" : "#93c5fd"
                      }`,
                      borderRadius: 999,
                      padding: "4px 10px",
                      fontSize: 12,
                  ...
                  <div style={{ flex: 1, fontSize: 13, color: "#334155" }}>
                    {x.t === "restaurant" ? (
                      <div
                        style={{
                          display: "grid",
                          gridTemplateColumns: "1.2fr 1fr 1fr 1.3fr 0.9fr",
                          gap: 8,
                          alignItems: "center",
                        }}
                      >
                        <div>
                          <strong>المتجر:</strong> {x.name || "-"}
                        </div>
                        <div>
                          <strong>المالك:</strong> {x.owner_name || "-"}
                        </div>
                        <div>
                          <strong>الهاتف:</strong> {x.phone || "-"}
                        </div>
                        <div>
                          <strong>العنوان:</strong> {x.address || "-"}
                        </div>
                        <div>
                          <strong>المحافظة:</strong> {x.province || "-"}
                        </div>
                      </div>
                    ) : (
                      <div
                      ...
                      >
                        ...
                        <div>
                          <strong>المركبة:</strong> {x.vehicle_type || "-"}
                        </div>
                        <div>
                          <strong>اللوحة:</strong> {x.vehicle_plate || "-"}
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <button
                    onClick={async () => {
                      await api.put(`/admin/approve/${x.t}/${x.id}`);
                      await load();
                    }}
                    style={miniBtn("#16a34a", "#fff")}
                  >
                    قبول
                  </button>
                  <button
                    onClick={async () => {
                      await api.put(`/admin/reject/${x.t}/${x.id}`);
                      await load();
                    }}
                    style={miniBtn("#dc2626", "#fff")}
                  >
                    رفض
                  </button>
                </div>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              >
                ...
                <textarea
                  ...
                  placeholder="سياسة التطبيق"
                  style={{ ...adminInputStyle, resize: "vertical" }}
                />
                <button type="submit" style={miniBtn("#111827", "#fff")}>
                  حفظ إعدادات الأدمن
                </button>
              </form>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  await Swal.fire({
                    icon: "success",
                    title: "تم تحديث بيانات دخول الأدمن",
                    confirmButtonColor: "#e31e24",
                  });
                  await load();
                }}
